...
command = "cat /etc/my.cnf | grep max_connections | awk -F'=' '{print $2}' | tr -d '[:space:]'"
mysql   = "mysql -h 127.0.0.1 -u monitor -p'#89623BHzYK79pS$' -Bse 'SELECT VARIABLE_VALUE FROM information_schema.GLOBAL_VARIABLES WHERE VARIABLE_NAME = \"max_connections\";' | tr -d '[:space:]'"
facter  = "curl http://169.254.169.254/latest/meta-data/instance-type"
